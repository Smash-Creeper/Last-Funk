...
local override_p1_color = override_colors and "FFEE00" or nil
...
function onCreatePost()
	...
	makeLuaSprite('healthBar', 'healthBar', 320, 629.55);
	addLuaSprite('healthBar', true)
	setObjectCamera('healthBar', "hud")	
	setObjectOrder('healthBar', 1)
	setLuaSpriteScrollFactor('healthBar', 5, 5);
	scaleObject('healthBar', 1, 1.50)    
	...
	if downscroll then
	setProperty('healthBar.y', 68)
end	
end

function onUpdatePost()
	local percent_p1 = (getProperty("healthBar.percent") / -100)
	local percent_p2 = -2 - percent_p1
